...
import core.convert.IConvertToDouble;
...
public class ConvertToDouble implements IConvertToDouble {
	...
	@Override
	public Double convert(byte[] bytes) throws IncorrectDataException, PermissibleErrorException {
		if (bytes == null) {
			return null;
		}
	...
	private Double decimalToDouble(byte[] bytes) throws PermissibleErrorException {
		...
		if (castError.compareTo(new BigDecimal(permissibleError)) == 1) {
			...
			throw new PermissibleErrorException("castError = " + castError);
		}
		return resultDouble;
